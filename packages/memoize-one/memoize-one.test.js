...
describe('memoizeOne', () => {
  ...
  describe('standard behaviour - baseline', () => {
    ...
    beforeEach(() => {
      add = jest.fn().mockImplementation((value1, value2) => value1 + value2);
  ...
  describe('throwing', () => {
    ...
    it('should not memoize a thrown result', () => {
      const willThrow = jest.fn().mockImplementation(message => {
        throw new Error(message);
      });
      const memoized = memoizeOne(willThrow);
      let firstError;
    ...
    it('should not break the memoization cache of a successful call', () => {
      const canThrow = jest.fn().mockImplementation(shouldThrow => {
        if (shouldThrow) {
  ...
  describe('flow typing', () => {
    it('should maintain the type of the original function', () => {
      ...
      const requiresASubtractFn = fn => fn(2, 1);

      const memoizedSubtract = memoizeOne(subtract);
